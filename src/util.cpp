...
const char *const MIRACLE_CONF_FILENAME = "miracle.conf";
const char *const MIRACLE_PID_FILENAME = "miracled.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Miracle
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Miracle
    // Mac: ~/Library/Application Support/Miracle
    // Unix: ~/.miracle
#ifdef WIN32
    // Windows
...
void ArgsManager::ReadConfigFile(const std::string &confPath)
{
    ...
    if (!streamConfig.good())
        return; // No miracle.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override miracle.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", MIRACLE_PID_FILENAME));
...
std::string CopyrightHolders(const std::string &strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Miracle Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Miracle Core") == std::string::npos)
    {
        strCopyrightHolders += "\n" + strPrefix + "The Miracle Core developers";
    }
