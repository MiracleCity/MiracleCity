...
static const std::regex PROTECTED_NAMES("^MCAM$|^MIRACLE$|^MCAMCOIN$|^MCAMHACOIN$|^MCAMHACHAIN$");
...
bool CNewToken::IsValid(std::string& strError, CTokensCache& tokenCache, bool fCheckMempool, bool fCheckDuplicateInputs, bool fForceDuplicateCheck) const
{
    ...
    if (units < 0 || units > 4) {
        strError = _("Invalid parameter: units must be between 0-4.");
        return false;
    }
...
void CNewToken::ConstructTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(TOKEN_MCAM);
...
{
    // Issuing an Token must contain at least 3 CTxOut( Miracle Burn Tx, Any Number of other Outputs ..., Owner Token Tx, New Token Tx)
    if (vout.size() < 3) {
...
{
    // Must contain at least 3 outpoints (MCAM burn, owner change and one or more new unique tokens that share a root (should be in trailing position))
    if (vout.size() < 3) {
...
{
    // Reissuing an Token must contain at least 3 CTxOut ( Miracle Burn Tx, Any Number of other Outputs ..., Reissue Token Tx, Owner Token Change Tx)
    if (vout.size() < 3) {
...
bool CReissueToken::IsValid(std::string &strError, CTokensCache& tokenCache, bool fForceCheckPrimaryTokenExists) const {
    ...
    if (nUnits > MAX_UNIT || nUnits < -1) {
        strError = _("Unable to reissue token: unit must be between 4 and -1");
        return false;
    }
...
{
    // Check the first transaction and verify that the correct MCAM Amount
...
{
    // Verify 'MCAMq' is in the transaction
...
{
    // Verify 'MCAMr' is in the transaction
    CScript scriptPubKey = txOut.scriptPubKey;
...
bool CreateTokenTransaction(CWallet* pwallet, CCoinControl& coinControl, const std::vector<CNewToken> tokens, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired)
{
    ...
    if (!change_address.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Miracle address: ") + change_address);
            return false;
        }
    } else {
    ...
    // Check to make sure the wallet has the MCAM required by the burnAmount
...
bool CreateReissueTokenTransaction(CWallet* pwallet, CCoinControl& coinControl, const CReissueToken& reissueToken, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired)
{
    ...
    if (!IsValidDestinationString(address)) {
        error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Miracle address: ") + address);
        return false;
    }
...
bool CreateTransferTokenTransaction(CWallet* pwallet, const CCoinControl& coinControl, const std::vector< std::pair<CTokenTransfer, std::string> >vTransfers, const std::string& changeAddress, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired)
{
    ...
    CAmount curBalance = pwallet->GetBalance();
    if (curBalance == 0) {
        error = std::make_pair(RPC_WALLET_INSUFFICIENT_FUNDS, std::string("This wallet doesn't contain any MCAM, transfering an token requires a network fee"));
        return false;
    }
