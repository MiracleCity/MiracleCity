...
BOOST_FIXTURE_TEST_SUITE(token_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of MIRACLECOIN NOT allowed
        BOOST_CHECK(!IsTokenNameValid("MCAM", type));
        BOOST_CHECK(!IsTokenNameValid("MIRACLE", type));
        BOOST_CHECK(!IsTokenNameValid("MIRACLECOIN", type));

        //- Versions of MIRACLECOIN ALLOWED
        BOOST_CHECK(IsTokenNameValid("MIRACLE.COIN", type));
        BOOST_CHECK(IsTokenNameValid("MIRACLE_COIN", type));
        BOOST_CHECK(IsTokenNameValid("MCAMSPYDER", type));
        BOOST_CHECK(IsTokenNameValid("SPYDERMCAM", type));
        BOOST_CHECK(IsTokenNameValid("MIRACLESPYDER", type));
        BOOST_CHECK(IsTokenNameValid("SPYDEMIRACLE", type));
        BOOST_CHECK(IsTokenNameValid("BLACK_MIRACLES", type));
        BOOST_CHECK(IsTokenNameValid("SEMCAMOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_token_coin_test)
    {
        ...
        CTokenTransfer token("MIRACLE", 1000, 0);
    ...
    BOOST_AUTO_TEST_CASE(new_token_coin_test)
    {
        ...
        // Create the token scriptPubKey
        CNewToken token("MIRACLE", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        token.ConstructTransaction(scriptPubKey);
