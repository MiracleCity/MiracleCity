...
#include "config/miracle-config.h"
#endif

#include "miraclegui.h"
...
const std::string MiracleGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString MiracleGUI::DEFAULT_WALLET = "~Default";

double GetPoSKernelPS();

MiracleGUI::MiracleGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...

{
    ...
    // MacDockIconHandler::instance()->setIcon(QIcon(":icons/about_mcam"));
#endif
...
void MiracleGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to an Miracle address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and miracle: URIs)"));
    ...
    transferTokenAction->setStatusTip(tr("Transfer tokens to MCAM addresses"));
    ...
    aboutAction = new QAction(platformStyle->TextColorIcon(":/icons/about_mcam"), tr("&About %1").arg(tr(PACKAGE_NAME)), this);
    ...
    toggleHideAction = new QAction(platformStyle->TextColorIcon(":/icons/about_mcam"), tr("&Show / Hide"), this);
    ...
    openAction->setStatusTip(tr("Open a miracle: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Miracle command-line options").arg(tr(PACKAGE_NAME)));
...
void MiracleGUI::createToolBars()
{
...
void MiracleGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool MiracleGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool MiracleGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void MiracleGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void MiracleGUI::setWalletActionsEnabled(bool enabled)
{
...
void MiracleGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void MiracleGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void MiracleGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void MiracleGUI::optionsClicked()
{
...
void MiracleGUI::showDebugWindow()
{
...
void MiracleGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void MiracleGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void MiracleGUI::openClicked()
{
...
void MiracleGUI::gotoOverviewPage()
{
    footerWidget->show();
    overviewAction->setChecked(true);
...
void MiracleGUI::gotoHistoryPage()
{
    footerWidget->hide();
...
void MiracleGUI::gotoReceiveCoinsPage()
{
    footerWidget->hide();
...
void MiracleGUI::gotoSendCoinsPage(QString addr)
{
    footerWidget->hide();
...
void MiracleGUI::gotoSignMessageTab(QString addr)
{
    footerWidget->hide();
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MiracleGUI::gotoVerifyMessageTab(QString addr)
{
    footerWidget->hide();
...
/** TOKENS START */
void MiracleGUI::gotoTokensPage()
{
    footerWidget->hide();
...
void MiracleGUI::gotoCreateTokensPage()
{
    footerWidget->hide();
...
void MiracleGUI::gotoManageTokensPage()
{
    footerWidget->hide();
...
void MiracleGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Miracle network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void MiracleGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MiracleGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void MiracleGUI::updateHeadersSyncProgressLabel()
{
...
void MiracleGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void MiracleGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
...
void MiracleGUI::changeEvent(QEvent *e)
{
...
void MiracleGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MiracleGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void MiracleGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& tokenName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (tokenName == "MCAM")
        msg += tr("Amount: %1\n").arg(MiracleUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(MiracleUnits::formatWithCustomName(tokenName, amount, MAX_TOKEN_UNITS, true));
...
void MiracleGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MiracleGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MiracleGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MiracleGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MiracleGUI::setHDStatus(int hdEnabled)
{
...
void MiracleGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void MiracleGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MiracleGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MiracleGUI::updateStakingIcon()
{
    if (walletFrame) {
...
void MiracleGUI::detectShutdown()
{
...
void MiracleGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void MiracleGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void MiracleGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(MiracleGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void MiracleGUI::subscribeToCoreSignals()
{
...
void MiracleGUI::unsubscribeFromCoreSignals()
{
...
void MiracleGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void MiracleGUI::toggleStakingActive()
{
    ...
    menu(0)
{
    ...
    QList<MiracleUnits::Unit> units = MiracleUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const MiracleUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(MiracleUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (MiracleUnits::Unit u : MiracleUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(MiracleUnits::name(u)), this);
...
{
    setText(MiracleUnits::name(newUnits));
}
