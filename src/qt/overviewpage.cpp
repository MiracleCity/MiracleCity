...
public:
    explicit TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(MiracleUnits::MCAM),
    ...
    tokendelegate(new TokenViewDelegate(platformStyle, this))
{
    ...
    ui->MCAMBalancesLabel->setStyleSheet(STRING_LABEL_COLOR);
    ...
    ui->MCAMBalancesLabel->setFont(GUIUtil::getTopLabelFont());
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& stake, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance, const CAmount& watchOnlyStake, const CAmount& lockedBalance)
{
    ...
    ui->labelBalance->setText(MiracleUnits::formatWithUnit(unit, balance, false, MiracleUnits::separatorAlways));
    ui->labelUnconfirmed->setText(MiracleUnits::formatWithUnit(unit, unconfirmedBalance + lockedBalance, false, MiracleUnits::separatorAlways));
    ui->labelImmature->setText(MiracleUnits::formatWithUnit(unit, immatureBalance, false, MiracleUnits::separatorAlways));
    ui->labelStake->setText(MiracleUnits::formatWithUnit(unit, stake, false, MiracleUnits::separatorAlways));
    ui->labelTotal->setText(MiracleUnits::formatWithUnit(unit, balance + unconfirmedBalance + immatureBalance + lockedBalance, false, MiracleUnits::separatorAlways));
    ui->labelWatchAvailable->setText(MiracleUnits::formatWithUnit(unit, watchOnlyBalance, false, MiracleUnits::separatorAlways));
    ui->labelWatchPending->setText(MiracleUnits::formatWithUnit(unit, watchUnconfBalance, false, MiracleUnits::separatorAlways));
    ui->labelWatchImmature->setText(MiracleUnits::formatWithUnit(unit, watchImmatureBalance, false, MiracleUnits::separatorAlways));
    ui->labelWatchTotal->setText(MiracleUnits::formatWithUnit(unit, watchOnlyBalance + watchUnconfBalance + watchImmatureBalance + watchOnlyStake, false, MiracleUnits::separatorAlways));
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("MCAM")
    updateDisplayUnit();
}
...
void OverviewPage::showTokens()
{
    if (AreTokensDeployed()) {
        ...
        // This keeps the MCAM balance grid from expanding and looking terrible when token balance is hidden
